...
function showArrows(index, arrows) {
  ...
  if (arrows[index].body.position.x > windowWidth || arrows[index].body.position.y >= windowHeight - 10) {
...
for(var i=0;i<playerArrows.length;i++){
  ...
  var cbaseCollision=Matter.SAT.collides(playerArrows[i].body,computerBase.body);
  var carcherCollision=Matter.SAT.collides(playerArrows[i].body,computerArcher.body);

  if(computerCollision.collided || cbaseCollision.collided || carcherCollision.collided){
...
function handleComputerArrowCollision() {
  ...
  for(var i=0;i<computerArrows.length;i++){
    ...
    var pbaseCollision=Matter.SAT.collides(computerArrows[i].body,playerBase.body);
    var parcherCollision=Matter.SAT.collides(computerArrows[i].body,playerArcher.body);
  
    if(playerCollision.collided || pbaseCollision.collided || parcherCollision.collided){
